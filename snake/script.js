...
for (let i = 0; i < 400; i++) {
    ...
    //cell.innerHTML = i //для разработки (УБРАТЬ В КОНЦЕ)
...
//дыры в стенах
/*const holeInWalls = () => {
    let isHoleBefore = false
    walls.forEach( (wall) => {
        const hole = Math.floor(Math.random() * 15) //рандомные дыры
        const index = Array.prototype.indexOf.call(walls, wall) //индекс каждой стены
        if (((hole === 1) && (index < 53) || (isHoleBefore)) && 
        (index > 19) && (index % 2 === 1)) { //дыры справа (рандом или раньше была стена)
            wall.style.backgroundColor = 'dimgrey'
            wall.className = 'cell hole'
            //сквозные дыры напротив
            walls[index - 1].style.backgroundColor = 'dimgrey'
            walls[index - 1].className = 'cell hole'
            if (isHoleBefore) isHoleBefore = false
            else if (!isHoleBefore) isHoleBefore = true
    }
})
}
holeInWalls()
...
const fieldPosition = () => {
    ...
    /*changeField.style.bottom = fieldHTML.style.top
    changeField.style.left = (docWidth - 170) / 2 + 'px'
    changeField.style.display = 'block'*/
}

//при нажатии на кнопку смены игрового поля
/*changeField.onclick = () => {
    holes.forEach( (hole) => {
        hole.style.backgroundColor = 'white'
        hole.className = 'cell wall'
    })
    holeInWalls()
...
//при проходе через дыру в стене !НЕ РАБОТАЕТ!
/*const inHole = (snakeFragment) => {
    if (snakeFragment.className === 'cell hole') {
        const index = Array.prototype.indexOf.call(holes, snakeFragment)
        snakeFragment.className = 'cell snakeBody'
        if (index % 2 === 1) {
...
const crash = () => {
    ...
    setTimeout (() => alert(`Вы проиграли со счетом ${points}, игра длилась ${timeInGame/60000} минут`),
    100)
...
//TO DO:
//0. проход через дыры насквозь !ДЫРЫ НЕ РАБОТАЮТ!